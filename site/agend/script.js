document.addEventListener('DOMContentLoaded', () => {
    ...
    const showAllAppointmentsClientButton = document.getElementById('showAllAppointmentsClient');
    ...
    const viewAndSelectAppointmentsButton = document.getElementById('viewAndSelectAppointmentsButton');
    const deleteAppointmentMessage = document.getElementById('deleteAppointmentMessage');
    ...
    function getBarberDaySchedule(barber, dateStr) {
        ...
        if (day === 0 || day === 1) { // Domingo (0) ou Segunda (1)
            return null;
        }
    ...
    function isSlotAvailable(barber, dateStr, timeStr) {
        ...
        const isWithinBarberHours = timeStr >= daySchedule.start && timeStr < daySchedule.end;
        const isWithinShopHours = timeStr >= BARBER_SHOP_OPENING_HOURS.start && timeStr < BARBER_SHOP_OPENING_HOURS.end;
        const isDuringBreak = (barberBreaks[barber] && barberBreaks[barber][dateStr] && barberBreaks[barber][dateStr].includes(timeStr));
        const isBooked = getStoredData('agendamentos', [])
                         .some(appt => appt.barber === barber && appt.date === dateStr && appt.time === timeStr);

        return isWithinBarberHours && isWithinShopHours && !isDuringBreak && !isBooked;
    }
    ...
    function updateAvailableTimes() {
        ...
        if (day === 0 || day === 1) {
            displayMessage('A barbearia está fechada aos domingos e segundas-feiras.', 'error');
            return;
        } else {
        ...
        if (!barberSchedule) {
            displayMessage(`Nenhum horário de trabalho definido para ${selectedBarber} nesta data.`, 'error');
            return;
        }

        const candidateTimes = generateTimeSlots(barberSchedule.start, barberSchedule.end);
        const availableSlots = candidateTimes.filter(time => isSlotAvailable(selectedBarber, selectedDate, time));

        if (availableSlots.length === 0) {
            displayMessage('Nenhum horário disponível para este barbeiro nesta data. Por favor, escolha outro.', 'error');
            return;
        }
    ...
    function showAllAppointments() {
        ...
        const list = stored.map((a, index) => `${index + 1}. Cliente: ${a.clientName}, Serviço: ${a.service}, Data: ${a.date} às ${a.time} com ${a.barber}`).join('\n\n');
    ...
    function deleteAppointment(indexToDelete) {
        ...
        if (indexToDelete >= 0 && indexToDelete < stored.length) {
            ...
            displayMessage(`Agendamento de ${deleted[0].clientName} em ${deleted[0].date} às ${deleted[0].time} com ${deleted[0].barber} foi apagado.`, 'success', deleteAppointmentMessage);
            updateAvailableTimes(); // Atualiza a lista de horários disponíveis no formulário de agendamento
        } else {
            displayMessage('Número de agendamento inválido.', 'error', deleteAppointmentMessage);
        }
    }
    ...
    function deleteBreak(barber, date, time) {
        if (barberBreaks[barber] && barberBreaks[barber][date]) {
            ...
            barberBreaks[barber][date] = barberBreaks[barber][date].filter(bTime => bTime !== time);
            ...
            if (barberBreaks[barber] && barberBreaks[barber][date] && barberBreaks[barber][date].length < initialLength) {
                 displayMessage(`Pausa de ${time} para ${barber} em ${date} foi apagada.`, 'success', barberPanelMessage);
            } else {
                 displayMessage(`Pausa ${time} para ${barber} em ${date} não encontrada.`, 'error', barberPanelMessage);
            }
            updateAvailableTimes(); // Atualiza a lista de horários disponíveis no formulário de agendamento
        } else {
            displayMessage(`Pausa para ${barber} em ${date} não encontrada.`, 'error', barberPanelMessage);
        }
    }
    ...
    scheduleButton.addEventListener('click', () => {
        ...
        if (!isSlotAvailable(selectedBarber, date, time)) {
            displayMessage('Este horário não está disponível para o barbeiro ou data selecionados. Por favor, escolha outro.', 'error');
            return;
        }
        ...
        if (phoneNumber) {
            ...
            displayMessage('Agendamento realizado! Redirecionando para o WhatsApp do barbeiro...', 'success');
    ...
    if (showAllAppointmentsClientButton) {
        showAllAppointmentsClientButton.addEventListener('click', showAllAppointments);
    }
    ...
    addBreakButton.addEventListener('click', () => {
        ...
        if (!selectedBarber || !breakDate || !breakTime) {
            displayMessage('Por favor, selecione o barbeiro, a data e a hora da pausa.', 'error', barberPanelMessage);
            return;
        }
        ...
        if (barberBreaks[selectedBarber][breakDate].includes(breakTime)) {
            displayMessage('Esta pausa já foi adicionada.', 'error', barberPanelMessage);
            return;
        }

        barberBreaks[selectedBarber][breakDate].push(breakTime);
        saveStoredData('barberBreaks', barberBreaks);
        displayMessage(`Pausa de ${breakTime} para ${selectedBarber} em ${breakDate} adicionada.`, 'success', barberPanelMessage);
    ...
    viewBreaksButton.addEventListener('click', () => {
        const selectedBarber = barberLoginSelect.value;
        if (!selectedBarber) {
            displayMessage('Por favor, selecione um barbeiro para ver as pausas.', 'error', barberPanelMessage);
            return;
        }
    ...
    deleteBreakButton.addEventListener('click', () => {
        ...
        if (!selectedBarber || !dateToDelete || !timeToDelete) {
            displayMessage('Selecione o barbeiro, a data e a hora da pausa para apagar.', 'error', barberPanelMessage);
            return;
        }
        
        // Pergunta ao usuário se ele tem certeza antes de apagar
        if (confirm(`Tem certeza que deseja apagar a pausa de ${timeToDelete} em ${dateToDelete} para ${selectedBarber}?`)) {
    ...
    saveJoaoSchedule.addEventListener('click', () => {
        ...
        if (!date || !start || !end) {
            displayMessage('Por favor, preencha a data, hora de início e hora de fim para João.', 'error', joaoScheduleMessage);
            return;
        }

        if (start >= end) {
            displayMessage('A hora de início deve ser anterior à hora de fim.', 'error', joaoScheduleMessage);
            return;
        }

        joaoSpecificSchedules[date] = { start, end };
        saveStoredData('joaoSpecificSchedules', joaoSpecificSchedules);
        displayMessage(`Horário de João para ${date} salvo: ${start} - ${end}`, 'success', joaoScheduleMessage);
        updateAvailableTimes();
    });
    ...
    viewAndSelectAppointmentsButton.addEventListener('click', () => {
        ...
        if (storedAppointments.length === 0) {
            displayMessage('Nenhum agendamento para apagar.', 'error', deleteAppointmentMessage);
            return;
        }

        // Cria uma lista numerada para o usuário escolher
        const appointmentList = storedAppointments.map((a, index) => 
            `${index + 1}. Cliente: ${a.clientName} - ${a.service} - ${a.date} às ${a.time} com ${a.barber}`
        ).join('\n');

        const input = prompt(`Digite o número do agendamento que deseja apagar:\n\n${appointmentList}\n\nDigite 'cancelar' para sair.`);

        if (input === null || input.toLowerCase() === 'cancelar') {
            displayMessage('Operação de exclusão cancelada.', 'error', deleteAppointmentMessage);
            return;
        }

        const indexToDelete = parseInt(input) - 1; // Ajusta para índice base 0

        if (isNaN(indexToDelete) || indexToDelete < 0 || indexToDelete >= storedAppointments.length) {
            displayMessage('Número de agendamento inválido. Tente novamente.', 'error', deleteAppointmentMessage);
            return;
        }
        
        // Confirmação final antes de apagar
        const confirmDelete = confirm(`Tem certeza que deseja apagar o agendamento:\n${storedAppointments[indexToDelete].clientName} - ${storedAppointments[indexToDelete].service} - ${storedAppointments[indexToDelete].date} às ${storedAppointments[indexToDelete].time}?`);
        
        if (confirmDelete) {
            deleteAppointment(indexToDelete);
        } else {
            displayMessage('Exclusão de agendamento cancelada.', 'error', deleteAppointmentMessage);
        }
    });
